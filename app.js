...
function renderLogin() {
  ...
  // Carrega branding da config mesmo sem estar logado
  sb.from('configuracoes').select('chave, valor').then(({ data }) => {
    if (!data) return;
    const c = Object.fromEntries(data.map(r => [r.chave, r.valor]));
    const parts = (c.evento_titulo || '').split(' ');
    const baby  = parts[parts.length - 1];
    if (baby)         setTextIfEl('brandName', baby);
    if (c.evento_data) {
      const d = new Date(c.evento_data + 'T12:00:00');
      setTextIfEl('chipDate', '📅 ' + d.toLocaleDateString('pt-BR', { day: '2-digit', month: 'long' }));
    }
    if (c.evento_local) setTextIfEl('chipLocal', '📍 ' + c.evento_local.split(',')[0]);
  });
}
...
async function renderApp() {
  ...
  setTextIfEl('evPix',   cfg.pix_chave   || '—');
...
function buildCard(g, comments) {
  ...
  const commHtml = comments.map(c => {
    ...
    const reactRow = reactHtml
      ? `<div class="comment-reactions">${reactHtml}</div>
         <div class="add-reaction-row" data-cid="${c.id}">
           ${['👍','❤️','😂','😮','😢'].map(e =>
             `<button class="reaction-btn" data-cid="${c.id}" data-emoji="${e}">${e}</button>`
           ).join('')}
         </div>`
      : `<div class="add-reaction-row" data-cid="${c.id}">
           ${['👍','❤️','😂','😮','😢'].map(e =>
...
async function toggleReaction(cid, emoji) {
  ...
  commCache = {};
  await renderGrid();
  // Reabre seções que estavam abertas
  document.querySelectorAll('.comments-body.open').forEach(cb => cb.classList.add('open'));
}
...
function openChoiceModal(gid, title, price) {
  ...
  el$('modalBody').innerHTML = `
    ...
    <div class="choice-option" id="optPix">
      ...
      <p>Chave: <strong>${esc(cfg.pix_chave)}</strong>${cfg.pix_nome ? ` (${esc(cfg.pix_nome)})` : ''}</p>
    </div>` : ''}
        ...
        <div class="admin-content">
          ...
          <div class="admin-panel" id="ap-escolhas">
            <div class="a-card">
              ...
              <div class="table-wrap">
                <table id="tChoices">
                  <thead><tr><th>Convidado</th><th>E-mail</th><th>Telefone</th><th>Presente</th><th>Pagamento</th><th>Valor</th><th>Mensagem</th><th>Data</th></tr></thead>
                  <tbody></tbody>
                </table>
              </div>
            </div>
...
function admShowPanel(id, btn) {
  ...
  const labels = { dashboard: 'Dashboard', presentes: 'Presentes', escolhas: 'Escolhas', convidados: 'Convidados', configuracoes: 'Configurações' };
...
function admRenderChoices() {
  ...
  tbody.innerHTML = adm.choices.map(c => `
    ...
    || `<tr><td colspan="8" style="text-align:center;padding:24px;color:var(--muted)">Sem escolhas</td></tr>`;
}
