...
const IMG_MAX_BYTES = 800_000; // 800 KB — limite para upload de imagens

// ── Estado global ─────────────────────────────────────
let sb        = null;
let me        = null;   // { ...supabase_user, perfil: {...} }
let gifts     = [];
let myChoice  = null;
let cfg       = {};
let commCache = {};
...
// ── Flag anti-double-render no boot ──────────────────
...
(async function boot() {
  ...
  // Restaura hash caso o utilizador tenha clicado "Atualizar" no banner
  const returnHash = sessionStorage.getItem('_returnHash');
  if (returnHash) { sessionStorage.removeItem('_returnHash'); window.location.hash = returnHash; }

  // Carrega ícone antes mesmo do login (não requer auth)
  sb.from('configuracoes').select('valor').eq('chave', 'icone_app').single()
    .then(({ data }) => { if (data?.valor) applyAppIcon(data.valor); });

  // ÚNICO ponto de entrada para auth — onAuthStateChange controla tudo.
  // A flag _bootHandled evita o double-render causado pelo Supabase v2
  // que dispara INITIAL_SESSION + SIGNED_IN em sequência na restauração.
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
      if (!session) { renderLogin(); return; }
      if (_bootHandled) return; // bloqueia o 2.º disparo redundante
      _bootHandled = true;
      me = session.user;
      await loadProfile();
      route();
  ...
  // Wake-up: verifica sessão ao voltar ao tab (Android Chrome suspende o JS em background)
  ...
  window.addEventListener('online',  () => tryAutoRecover());
  window.addEventListener('offline', () => showBanner('Sem conexão com a internet. Aguardando reconexão...'));
})();
...
// Wrapper de chamada ao Supabase — intercepta erros de auth e aciona recuperação
async function sbCall(fn) {
  try {
    const result = await fn();
    if (result?.error) {
      const { code, status, statusCode, message = '' } = result.error;
      const httpCode = code ?? status ?? statusCode;
      const isAuth = httpCode === 401 || httpCode === 403
        || /JWT|expired|session|not authenticated/i.test(message);
      if (isAuth) tryAutoRecover();
    } else {
      _retryCount = 0;
      hideBanner();
    }
...
async function tryAutoRecover() {
  ...
  if (_retryCount <= MAX_RETRIES) {
    setTimeout(tryAutoRecover, RETRY_DELAY_MS);
  } else {
    showBanner();
  }
}
...
function showBanner(msg) {
  ...
  _bannerVisible = true;
  document.getElementById('_reconect_banner')?.remove();

  const banner = document.createElement('div');
  banner.id = '_reconect_banner';
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#b45309;color:#fff;font-family:inherit;padding:10px 16px;display:flex;align-items:center;justify-content:space-between;gap:12px;font-size:14px;box-shadow:0 2px 12px rgba(0,0,0,.25)';
  banner.innerHTML = `
    <span style="flex:1">⚠️ ${msg || 'Conexão encerrada. Clique para reconectar.'}</span>
    <button id="_reconect_btn" style="background:#fff;color:#b45309;border:none;border-radius:6px;padding:6px 14px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;flex-shrink:0">🔄 Atualizar página</button>`;
  document.body.prepend(banner);
  document.body.style.paddingTop = (parseInt(document.body.style.paddingTop || 0) + 44) + 'px';
  el$('_reconect_btn').addEventListener('click', () => {
    sessionStorage.setItem('_returnHash', window.location.hash || '');
    location.reload();
  });
}
...
function hideBanner() {
  ...
  const banner = el$('_reconect_banner');
  if (banner) {
...
function applyAppIcon(valor) {
  ...
  const mkImg = (size, r) => isB64
    ? `<img src="${valor}" style="width:${size}px;height:${size}px;object-fit:contain;border-radius:${r}px">`
    : valor;
...
function renderLogin() {
  ...
  // Branding dinâmico — carrega do banco sem bloquear o render
...
async function renderGrid() {
  ...
  const commMap = Object.fromEntries(gifts.map((g, i) => [g.id, allComments[i]]));
...
async function loadComments(pid) {
  ...
  if (error) console.warn('[comentarios]', error.message);
  commCache[pid] = data || [];
...
// Máscara de dinheiro — digita centavos da direita para esquerda
...
async function renderAdmin() {
          ...
          <div class="admin-panel active" id="ap-dashboard">
            ...
            <div class="a-card">
              ...
              <div class="dash-activity-list" id="dashActivity">
                <div style="text-align:center;padding:24px;color:var(--muted);font-size:13px">Carregando...</div>
              </div>
            </div>
          </div>
            ...
            <div class="cfg-sections">
              ...
              <div class="cfg-section">
                <div class="cfg-section-header">
                  <div class="cfg-section-icon">📅</div>
                  <div class="cfg-section-title">Informações do Evento</div>
                </div>
              ...
              <div class="cfg-section">
                <div class="cfg-section-header">
                  <div class="cfg-section-icon">🔐</div>
                  <div class="cfg-section-title">Tela de Boas-vindas</div>
                </div>
                <div class="cfg-section-body">
                  <div class="field">
                    <label>Subtítulo</label>
                    <textarea id="cfgLoginSubtitle" rows="2" placeholder="Ex: Venha celebrar esse momento especial..."></textarea>
                  </div>
                  <div class="field">
                    <label>Chips de destaque <small style="color:var(--muted)">(um por linha)</small></label>
              ...
              <div class="cfg-section">
                <div class="cfg-section-header">
                  <div class="cfg-section-icon">💸</div>
                  <div class="cfg-section-title">Configurações PIX</div>
                </div>
                <div class="cfg-section-body">
              ...
              <div class="cfg-section">
                <div class="cfg-section-header">
                  <div class="cfg-section-icon">🖼️</div>
                  <div class="cfg-section-title">Imagem de Capa</div>
                </div>
                <div class="cfg-section-body">
  ...
  el$('cfgCover')?.addEventListener('change', function () {
    ...
    r.onload = e => {
      ...
      const wrap = el$('cfgCoverWrap');
      const area = el$('cfgCoverUploadArea');
  ...
  window.previewIconInput = function (v) {
    ...
    const prev = el$('iconPreviewBig');
    const btn  = el$('btnRemoveIcon');
...
async function admLoadAll() {
  const [r1, r2, r3, r4, r5, r6] = await Promise.all([
    sbCall(() => sb.from('presentes').select('*').order('ordem')),
    sbCall(() => sb.from('escolhas').select('*, perfis(nome,email,telefone), presentes(titulo,preco)').order('criado_em', { ascending: false })),
    sbCall(() => sb.from('perfis').select('*, escolhas(presente_id, tipo_pagamento, presentes(titulo))').order('criado_em')),
    sbCall(() => sb.from('configuracoes').select('chave, valor')),
    sbCall(() => sb.from('contribuicoes_pix').select('*, perfis(nome,email)').order('criado_em', { ascending: false })),
    sbCall(() => sb.from('comentarios').select('id, comentario, criado_em, usuario_id, presente_id, perfis(nome), presentes(titulo)').order('criado_em', { ascending: false }))
  ]);

  if (r1.error) console.error('[adm presentes]',  r1.error.message);
  if (r2.error) console.error('[adm escolhas]',    r2.error.message);
  if (r3.error) console.error('[adm perfis]',      r3.error.message);
  if (r6.error) console.error('[adm comentarios]', r6.error.message);

  adm.gifts    = r1.data || [];
  adm.choices  = r2.data || [];
  adm.users    = r3.data || [];
  adm.pix      = r5.error ? [] : (r5.data || []);
  adm.comments = r6.error ? [] : (r6.data || []);
  adm.cfg      = {};
...
async function admSaveGift() {
  ...
  if (file && file.size > IMG_MAX_BYTES) {
    toast('Imagem muito grande. Máximo 800KB ou use URL.', 'error');
    return;
  }
  ...
  let error;
  if (id) {
    // Ao editar, recalcula quantidade_restante com base nas escolhas atuais
    const { data: escolhasAtuais } = await sb.from('escolhas')
      .select('quantidade').eq('presente_id', id);
...
async function admChoiceDelete(id, nome) {
  ...
  // Busca a escolha antes de deletar para saber qual presente atualizar
  const { data: escolha } = await sb.from('escolhas')
    .select('presente_id, quantidade')
    .eq('id', id).maybeSingle();

  const { error } = await sb.from('escolhas').delete().eq('id', id);
  if (error) { toast('Erro: ' + error.message, 'error'); return; }

  // Recalcula quantidade_restante com base nas escolhas restantes
  if (escolha?.presente_id) {
    const { data: present } = await sb.from('presentes')
      .select('quantidade_max').eq('id', escolha.presente_id).maybeSingle();
    const { data: escolhasRestantes } = await sb.from('escolhas')
      .select('quantidade').eq('presente_id', escolha.presente_id);
    ...
    await sb.from('presentes')
      .update({ quantidade_restante: novaRestante })
      .eq('id', escolha.presente_id);
  }
...
function admRenderConfig() {
  ...
  const btnRemove = el$('btnRemoveIcon');
  if (prevEl) {
  ...
  const coverWrap = el$('cfgCoverWrap');
  const coverPrev = el$('cfgCoverPrev');
  const coverArea = el$('cfgCoverUploadArea');
...
async function admSaveConfig() {
  ...
  // Upserts em paralelo (mais rápido que sequencial)
  const now = new Date().toISOString();
  await Promise.all(pairs.map(([chave, valor]) =>
