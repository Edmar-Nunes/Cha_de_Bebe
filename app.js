...
(async function boot() {
  ...
  if (session) { me = session.user; await loadProfile(); _bootDone = true; route(); }
  else { _bootDone = true; renderLogin(); }

  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'SIGNED_IN' && session) {
      if (!_bootDone) return; // sessão já tratada pelo getSession acima
      if (me?.id === session.user.id && me?.perfil) return; // já está logado com perfil, ignora disparo redundante
      me = session.user; await loadProfile(); route();
    } else if (event === 'SIGNED_OUT') {
      resetState(); renderLogin();
    }
  });

  // Wake-up: Android Chrome suspende JS em background.
  // Ao voltar ao tab, força refresh da sessão para descongelar as queries.
  document.addEventListener('visibilitychange', async () => {
    if (document.visibilityState !== 'visible') return;
    if (!me) return; // utilizador não está logado, nada a fazer
    const { data: { session } } = await sb.auth.getSession();
    if (session) {
      me = session.user;
      if (!me.perfil) await loadProfile();
    } else {
      // Sessão expirou enquanto estava em background
      resetState();
      renderLogin();
...
// Wrapper sbCall — passa directo sem interceptor de reconexão
